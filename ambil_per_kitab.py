...
with open("C:\\Users\\Totz Tech\\Documents\\Code\\alkitab.txt",'r') as f:
    alkitab = f.read()

import time


keys = list(bible_structure.keys())
print("panjang keys: " + str(len(keys)))


for i, key in enumerate(keys):
    string_awal = key +" 1"
    if i + 1 == len(keys):
        indeks_awal = alkitab.find(string_awal)
        indeks_akhir = len(alkitab)
        data_per_kitab = alkitab[indeks_awal:indeks_akhir]
        bible_structure[key]["data_per_kitab"] = data_per_kitab
    else:
        indeks_awal = alkitab.find(string_awal)
        string_akhir = keys[i+1]
        indeks_akhir = alkitab.find(string_akhir)
        data_per_kitab = alkitab[indeks_awal:indeks_akhir]
        bible_structure[key]["data_per_kitab"] = data_per_kitab

import json
print(bible_structure)
with open("C:\\Users\\Totz Tech\\Documents\\Code\\struktur_alkitab1.json",'w') as f:
    json.dump(bible_structure, f)
